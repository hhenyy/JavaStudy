...
//2.자식 클래스로 객체를 생성한 다음에 메소드를 호출하면, 메소드 오버라이딩된 메소드만 호출
//3.부모 클래스로 상속 해주는 메소드는 더 이상 사용할 수 없는 은닉 메소드가 된다.
//4.부모 클래스의 은닉 메소드를 사용하기 위해서는 자식클래스의 메소드안에서 super.parentPrn() 형식으로 호출해서 사용할 수 있다.
...
class Child05 extends Parent05{  //자식 클래스
  ...
  public void parentPrn(){ //메소드 재정의(Method Overriding): 메소드 이름과 형식은 동일하게 정의,내용은 다르게함.(이름과 형식이 다르면 새 메소드를 만드는것)
